...
sub SIGNALduino_OSV2 {
	...
	if ($bitData =~ m/^.?(01){12,17}.?10011001/)
	{
		...
		return (-1," message with length ($message_length) is to short") if ($message_length < $hash->{protocolObject}->checkProperty($id,'length_min',-1) );
