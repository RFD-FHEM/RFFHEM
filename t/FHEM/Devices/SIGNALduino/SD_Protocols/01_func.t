...
subtest 'lib SD_Prococols test sub LoadHash() ' => sub {
	...
	$ret = $Protocols->LoadHash(
		'./t/FHEM/Devices/SIGNALduino/SD_Protocols/test_loadprotohash-nok.pm');
	like( $ret, qr/syntax error at/, 'file with syntax error' );

	$ret = $Protocols->LoadHash(
		'./t/FHEM/Devices/SIGNALduino/SD_Protocols/test_loadprotohash-ok.pm');
...
subtest 'lib SD_Prococols test sub new() ' => sub {
	...
	$Protocols = new $className( filename => './t/FHEM/Devices/SIGNALduino/SD_Protocols/test_loadprotohash-nok.pm' );
	is( $Protocols->{_protocols}, U(), 'file with syntax error' );

	$Protocols = new $className( filename => './t/FHEM/Devices/SIGNALduino/SD_Protocols/test_loadprotohash-ok.pm' );
...
subtest 'lib SD_Prococols protocolExists()' => sub {
	...
	subtest 'from json' => sub {
		plan(5);
		my $Protocols =
		  new $className( filetype => 'json', filename => './t/FHEM/Devices/SIGNALduino/SD_Protocols/test_protocolData.json' );
	...
	subtest 'from PerlModule' => sub {
		plan(5);
		my $Protocols =
		  new $className( filename => './t/FHEM/Devices/SIGNALduino/SD_Protocols/test_loadprotohash-ok.pm' );
...
subtest 'lib SD_Prococols getKeys() with filter' => sub {
	plan(2);
	my $Protocols =	new $className( filetype => 'json', filename => './t/FHEM/Devices/SIGNALduino/SD_Protocols/test_protocolData.json' );
...
subtest 'lib SD_Prococols setDefaults()' => sub {
	...
	subtest 'method default for MC protocol' => sub {
		...
		is($Protocols->{_protocols}->{9989}->{method},\&FHEM::Devices::SIGNALduino::SD_Protocols::MCRAW,'verify method is default coderef');
	};
	subtest 'method explicit specified' => sub {
		plan(3);
		$Protocols->{_protocols}->{9989}->{method}='FHEM::Devices::SIGNALduino::SD_Protocols::ConvHE800';
		is($Protocols->setDefaults,U(),'verify return value');
		ref_ok($Protocols->{_protocols}->{9989}->{method},'CODE','verify method is a coderef now');
		is($Protocols->{_protocols}->{9989}->{method},\&FHEM::Devices::SIGNALduino::SD_Protocols::ConvHE800,'verify method is ConvHE800 coderef');
	};	
};
