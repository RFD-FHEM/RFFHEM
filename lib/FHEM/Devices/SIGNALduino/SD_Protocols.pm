...
#
################################################################################
package FHEM::Devices::SIGNALduino::SD_Protocols;
...
############################# package FHEM::Devices::SIGNALduino::SD_Protocols, test exists
=item LoadHash()
...
sub LoadHash {
  ...
  $self->{_protocols}        = \%FHEM::Devices::SIGNALduino::SD_Protocols::Data::protocols;
  $self->{_protocolsVersion} = $FHEM::Devices::SIGNALduino::SD_Protocols::Data::VERSION;
  ...
  for my $id ( $self->getKeys() )
  {
    ...
    if ( defined $format && ($format eq 'manchester' || $format =~ 'FSK') )
    {
      ...
      ( !defined $cref && $format eq 'manchester' )
        ...
        \&FHEM::Devices::SIGNALduino::SD_Protocols::MCRAW
        : undef;
...
sub mcBit2AS {
  ...
  {  # Valid AS detected!
    ...
    my $ashex = FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr($msgbits); # output with length before
...
sub mcBit2Grothe {
  ...
  my $hex = FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr($bitData);
...
sub mcBit2Maverick {
  ...
  {  # Valid Maverick header detected
    ...
    my $hex=FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr(substr($bitData,$header_pos,26*4));
    return  (1,$hex); ## Return the bits unchanged in hex
  } else {
...
sub mcBit2OSV1 {
  ...
  if ($channel eq '00') {                          # in 0 LSB first
    ...
    my $osv1hex = '50' . FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr($newBitData); # output with length before
...
sub mcBit2SomfyRTS {
  ...
  my $encData = FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr($bitData);
...
sub mcBit2TFA {
  ...
  if ($bitData =~ m/(1{9}101)/xms )
  {
    ...
    do
    {
      ...
      if ($rcode) {
        my $hex=FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr($part_str);
        push (@messages,$hex);
...
sub postDemo_Revolt {
  ...
  if ($sum != $checksum) {
    my $dmsg = FHEM::Devices::SIGNALduino::SD_Protocols::binStr2hexStr( join "", @bit_msg[ 0 .. 95 ] );
...
sub ConvBresser_7in1 {
  ...
  my $checksum = FHEM::Devices::SIGNALduino::SD_Protocols::LFSR_digest16(21, 0x8810, 0xBA95, substr($hexDataXorA,4,42));
...
sub ConvBresser_lightning {
  ...
  my $checksum = FHEM::Devices::SIGNALduino::SD_Protocols::LFSR_digest16(8, 0x8810, 0xABF9, substr($hexDataXorA,4,16));
  my $checksumcalc = sprintf('%04X',$checksum ^ hex(substr($hexDataXorA,0,4)));
...
############################# package FHEM::Devices::SIGNALduino::SD_Protocols, test not exists
