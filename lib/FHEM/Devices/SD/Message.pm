...
package FHEM::Devices::SD::Message;
...
require FHEM::Devices::SD::Logger;
require FHEM::Devices::SD::Matchlist; 
require FHEM::Devices::SD::Clients;
...
sub Dispatch {
  ...
  if (!defined($dmsg))
  {
    ...
    FHEM::Devices::SD::Logger::Log($hash, 5, "$name: Dispatch, dmsg is undef. Skipping dispatch call");
    return;
  }
  ...
  if ($dmsg eq $hash->{LASTDMSG}) {
    # Logging-Aufruf ersetzt (Aktualisiert)
    FHEM::Devices::SD::Logger::Log($hash, SDUINO_DISPATCH_VERBOSE, "$name: Dispatch, $dmsg is equal to last DMSG");
  } else {
    if ( defined $hash->{DoubleMsgIDs}{$id} ) {
      $DMSGgleich = 0;
      # Logging-Aufruf ersetzt (Aktualisiert)
      FHEM::Devices::SD::Logger::Log($hash, SDUINO_DISPATCH_VERBOSE, "$name: Dispatch, $dmsg is unequal to last DMSG (DoubleMsgID is enabled)");
    } else {
      # Logging-Aufruf ersetzt (Aktualisiert)
      FHEM::Devices::SD::Logger::Log($hash, SDUINO_DISPATCH_VERBOSE, "$name: Dispatch, $dmsg, is unequal to last DMSG (DoubleMsgID is disabled)");
    }
    $hash->{LASTDMSG} = $dmsg;
    $hash->{LASTDMSGID} = $id;
  }
  ...
  if ($DMSGgleich) {
        ...
        && ( !defined $hash->{protocolObject} || $hash->{protocolObject}->checkProperty($id,'dispatchequals','false') ne 'true' ) )
    {
      ...
      FHEM::Devices::SD::Logger::Log($hash, SDUINO_DISPATCH_VERBOSE, "$name: Dispatch, $dmsg, $rssi dispatch");
      main::Dispatch($hash, $dmsg, \%addvals); 
    } else {
      FHEM::Devices::SD::Logger::Log($hash, 4, "$name: Dispatch, $dmsg, Dropped due to short time or equal msg");
    }
  }
}
...
sub json2Dispatch {
  ...
  if (!defined($json_str) || !defined($name)) {
    FHEM::Devices::SD::Logger::Log($name, 3, "json2Dispatch: Missing arguments (JSON or Name)");
    return;
  }

  my $hash = $main::defs{$name}; 
  if (!defined($hash)) {
    FHEM::Devices::SD::Logger::Log($name, 3, "json2Dispatch: Device $name not found");
    return;
  }
  ...
  if ($@) {
    FHEM::Devices::SD::Logger::Log($name, 3, "json2Dispatch: JSON decode error: $@");
    return;
  }
  ...
  if (!defined($message)) {
     FHEM::Devices::SD::Logger::Log($name, 4, "json2Dispatch: Missing 'data' in JSON structure");
     return;
  }
  
  if (!defined($json->{protocol}) || !defined($json->{protocol}->{id})) {
     FHEM::Devices::SD::Logger::Log($name, 4, q[json2Dispatch: Missing ' "protocol":{id:}" ' in JSON structure]);
     return;
  }
  ...
  if (!defined($dmsg) || !defined($id)) {
     FHEM::Devices::SD::Logger::Log($name, 4, "json2Dispatch: No dmsg could be created from JSON");
     return;
  }
  
  if (!defined($hash->{MatchList}) || !defined($hash->{Clients}) ) {
    FHEM::Devices::SD::Logger::Log($hash, 4, "json2Dispatch: Matchlist/Clientlist initialization");
    
    FHEM::Devices::SD::Matchlist::UpdateMatchList($hash,undef);
    $hash->{Clients} = FHEM::Devices::SD::Clients::getClientsasStr($hash);
  }

  # Call central dispatch function
  FHEM::Devices::SD::Logger::Log($hash, 5, "json2Dispatch: Calling FHEM Dispatch with dmsg=$dmsg, id=$id");
