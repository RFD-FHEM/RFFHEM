# $Id: 00_SIGNALduino.pm 0 2026-01-15 18:07:14Z sidey79 $
...
use FHEM::Devices::SD::Clients;
use FHEM::Devices::SD::Message;
use FHEM::Devices::SD::Matchlist;
...
use constant {
  SDUINO_VERSION                  => '3.5.7+20260115',  # Datum wird automatisch bei jedem pull request aktualisiert
...
sub SIGNALduino_Define {
  ...
  $hash->{Clients}    = FHEM::Devices::SD::Clients::getClientsasStr();
  $hash->{MatchList}  = FHEM::Devices::SD::Matchlist::getMatchListasRef();
  $hash->{DeviceName} = $dev;
  ...
  if (defined($clockidx) and defined($syncidx))
  {
    ...
    foreach my $id (@{$hash->{msIdList}}) {
      ...
      if ( SIGNALduino_moduleMatch($name,$id,$dmsg) == 1)
      {
        ...
        FHEM::Devices::SD::Message::Dispatch($hash,$rmsg,$dmsg,$rssi,$id);
      }
    }
...
sub SIGNALduino_Parse_MN {
  ...
  for my $id (@{$hash->{mnIdList}}) {
    ...
    FHEM::Devices::SD::Message::Dispatch($hash,$rmsg,$dmsg,$rssi,$id,$freqafc);
    $message_dispatched++;
...
sub SIGNALduino_Attr {
  ...
  ## Change MatchList
    ...
    FHEM::Devices::SD::Matchlist::UpdateMatchList($hash,$match_list);
  }
  ## Change verbose
...
sub SIGNALduino_IdList {
  ...
  if ($wflag == 0) {                      # whitelist not active
    ...
    $hash->{Clients} = AttrVal($name,'Clients', FHEM::Devices::SD::Clients::getClientsasStr()); # use Attribute Clients or default if whitelist is not active
  } else {
